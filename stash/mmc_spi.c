...
enum MMC_EC {
	MMC_EC_OK = 0, MMC_EC_NOT_IDLE = 1, //after reset command, device wasn't in idle state
...
MMC_EC mmc_modify_block(int32 address, int16 size, int *ptr) {
	...
	while (size) {
		...
		if ((size + i) > MMC_BLOCK_SIZE) {
			num = MMC_BLOCK_SIZE - i;
		} else {
			num = size;
		}
...
MMC_EC mmc_erase(int32 address, int32 sectors) {
	...
	if (sectors) {
		...
		mmc_deselect();
		if (res) {
			return (MMC_EC_ERASE_SECTOR_START_FAIL);
		}
		...
		mmc_deselect();
		if (res) {
			return (MMC_EC_ERASE_SECTOR_END_FAIL);
		}
		...
		mmc_deselect();
		if (res) {
			return (MMC_EC_ERASE_SECTORS_FAIL);
		}
	}
	return (MMC_EC_OK);
}
